...
class Optimizer():
    ...
    def solve_simulations(self, params, solver="fixed_point"):
        ...
        reaction_list = self.update_reaction_list(self.simulator.output_parser['reactions_list'], dict_new_vec)
        
        ###* compute the new rates and solve the system of equations
        rates_calculation_arr = self.simulator.compute_rates(reactions_list)
        frac_solutions_arr, gammas_result_arr = self.simulator.solve_system(rates_calculation_arr, solver=solver)
        gammas_sum_arr = np.array([sum(gamma_dict.values()) for gamma_dict in gammas_result_arr])
        
        return frac_solutions_arr, gamma_results_arr, gammas_sum_arr
    ...
    def functional_loss(self, params):
        ...
        ###* define the dictionaries with the params to update
        dict_new_vec = self.func_new_model_dict(params)
        # reactions_list = self.simulator.output_parser['reactions_list']
    ...
    def hybrid_search(self, config, nb_workers=2, print_flag=False):
        ...
        result = sp.optimize.differential_evolution(self.functional_loss, bounds, 
                                                    polish=True,
                                                    disp=True,
                                                    workers=pool.map,
                                                    updating="deferred",
