...
class Simulator():
    ...
    def compute_rates(self, reaction_list):
        rates_calculation_arr = self.sim_rates.compute_rates_simulation(self.exp_data_arr, reaction_list)
    ...
    def compute_gammas(self, steady_solutions_arr, rates_calculation_arr, S_specie = 'S', O2_specie='O2'):
    ...
    def solve_system(self, rates_calculation_arr, solver="fixed_point", max_time=15):
        
        frac_steady_sol_arr = np.zeros((len(self.exp_data_arr), len(self.output_parser['species_model'])), dtype=float)
        for counter in range(self.exp_data_arr.shape[0]):
            ...
            if success:
                ...
                frac_steady_sol_arr[counter] = sol
                print(f"In {self.exp_data_arr[counter]} conditions, the solver failed to converge")
                
        ###* compute gammas
        gammas_result_arr = self.compute_gammas(frac_steady_sol_arr, rates_calculation_arr)
