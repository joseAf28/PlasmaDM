...
def run_local_optimization(params_arr, opt_object, diff_object):
...
if __name__ == '__main__':
    ...
    n_samples = 100
    output_file = f"results_grad_{n_samples}_merged_AllV2.h5"
    num_workers = 8
    ...
    with ProcessPool(nodes=num_workers) as pool:
        results = list(tqdm.tqdm(
                    pool.imap(
                        lambda params: run_local_optimization(params, optimizer, diff),
                        samples
                ),
                total=len(samples),
                desc="Optimizing"
    ...
    for ele in results:
        fun_samples.append(ele.fun)
        params_samples.append(ele.x)
        iters_samples.append(ele.nfev)
    
    
    with h5py.File(output_file, "w") as f:
        f.create_dataset("fun", data=fun_samples)
        f.create_dataset("params", data=params_samples)
        f.create_dataset("iters", data=iters_samples)
    
    
    print("fun_samples: ", fun_samples)
    print("params_samples: ", params_samples)
    print("iters_samples: ", iters_samples)